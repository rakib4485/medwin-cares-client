      ...
      <div className='my-14 grid grid-cols-1 items-center lg:grid-cols-2'>
        ...
        <div className='h-[800px] flex justify-center items-center'>
          ...
          <form onSubmit={handleSubmit(handleLogin)}>
            ...
            <input {...register("email", {required: "Email Address is required"})} defaultValue="admin@gmail.com" type="text" className="input input-bordered w-full max-w-xs"/>
            {errors.email && <p className='text-red-600'>{errors.email?.message}</p>}
            </div>
            <div className="form-control w-full max-w-xs">
              <label className="label"><span className="label-text">Password</span></label>
            <input defaultValue="bayazid" {...register("password", {required: "Password is required",  minLength: {value: 6, message: 'Password must be 6 characters or longer'}})} type="password" className="input input-bordered w-full max-w-xs"/>
